...
* Create `DistanceValueFunction` that replace `ValueDistFunction` and implements the interface `PartialValueFunction<LatLng>` according to the UML implementation at iteration 4.
* Reduce the redundancies in the `AVF` class to simplify the code : Regroup the setters and getters in generic methods. 
* Try to integrate the distance criteria in the `AVF` class according to the documentation done in the iteration 4 of UML.

==== Stellan WEA & Samy MEGUENI

* Improving the `SpiderPlot` code by following professor's advices : formating, test, fail fast, static factory methods, and the other trails of improvement as requested in the link:https://github.com/oliviercailloux-org/projet-apartments/pull/22[PR #22].
* Generating image instead of displaying the plot with a swing panel. Display the result will be in a future iteration.
* For the spiderplot, taking existing data from apartments of the project instead of generating them by hand. According to your suggestion in the link:https://github.com/oliviercailloux-org/projet-apartments/pull/22[PR #22]
