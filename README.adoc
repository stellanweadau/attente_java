...
* SM/PO for this iteration.
* Have split the work between the two groups.
* Improvement of previous PRs after the first review.
* Made sure everybody was on board and respected the deadline.

==== Abla MIKOU & Seif BOUGUERRA

* Update the `DC_SpiderPlot` by integrating it in the `diagram-package-global`. link:https://github.com/oliviercailloux-org/projet-apartments/pull/20[(PR #20)] 
* Integrate the distance criterias (implementation currently in java) in all the diagrams.
* Explain how the `ValueDistFunction` is used in the `AVF` by modifying existing diagrams.


==== Maïrame NDIAYE & Stellan WEA

* Update the `SD_ValueFunction` diagram of the previous iteration according to the professor's advices. link:https://github.com/oliviercailloux-org/projet-apartments/pull/19[(PR #19)]
* Modify the `SD_Distance` diagram from the second iteration according to the professor's advices and our 
fresh vision of the distance code. link:https://github.com/oliviercailloux-org/projet-apartments/pull/11[(PR #11)]
