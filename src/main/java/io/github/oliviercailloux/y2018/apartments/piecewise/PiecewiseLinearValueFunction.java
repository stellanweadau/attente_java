...
/**
 * 
 * The public class PricewiseLinearValueFunction enables the creation of an
 * object with a criteria which represents a property of an apartment, and two
 * numbers which represent first the value of the criteria ( for example the
 * number of rooms ) and the utility associated to this criteria. The class
 * enables the user to know for any value of the criteria, the utility
 * associated.
 */
public class PiecewiseLinearValueFunction implements IPiecewiseLinearValueFunction {
	 ...
	 * 
	 * @param crit Label of the criteria
	 */
	...
	public void setUtility(int key, double value) {
		...
		if (key < 0 || value < 0 || value > 1) {
			...
			if (value < 0 || value > 1)
				LOGGER.error(
						"The key has not been set because the value of the utility doesn't respect the constraints.");
	...
	public String toString() {
		return "This object is defining the following criteria :  " + criteria + ". It contains "
				+ this.breakPoints.size() + " values";
	}

	/**
	 * This method calculates the linear value of a function. If the point A and the
	 * point B are not different, the method returns an IllegalArgumentException.
	 * 
	...
	private double getLinearValue(Point2D a, Point2D b) {
		...
		if (a.getX() == b.getX()) {
			LOGGER.error("The linear value cannot be calculated.The abscisse of the point A " + a.getX()
					+ " can't be the same than the abscisse of the point B " + b.getX());
	...
	public Range<Integer> getInterval(int key) throws IOException {
		...
		if (key > getMaxKey()) {
			LOGGER.error("The key value " + key + " is more than " + getMaxKey()
					+ ". No coherent value to return for the range.");
			throw new IllegalArgumentException("No coherent value to return for the range");
		}

		if (key < getMinKey()) {
			LOGGER.error("The key value " + key + " is less than " + getMinKey()
					+ ". No coherent value to return for the range.");
			throw new IllegalArgumentException("No coherent value to return for the range");
		}
